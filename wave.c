...
// dispWAVdata function displays 80 pieces of RMS value on termminal screen, but
// this amount of data is still too much to send. According to the RMS standard,
...
void dispWAVdata(short int s[]){
	...
	for(i=0; i<80; i++){		// outer loop for 80 times
		...
		if(i%10==9){		// if we have done accumulation 10 times
			...
			sum8 = 0.0;	// reset sum8 fot next Leq value
		}
